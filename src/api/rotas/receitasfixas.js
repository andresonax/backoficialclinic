...
module.exports = function(server) {
    ...
    //READ ALL
    var sql = `SELECT receitasfixas.*, fornecedor.razao_social, categoriafinanceiro.descricao as desc_financeiro FROM receitasfixas `+
    ` inner join categoriafinanceiro on receitasfixas.id_categoria = categoriafinanceiro.id_categoria ` +
    ` inner join fornecedor on receitasfixas.id_cli_for = fornecedor.id_fornecedor `
    router.get('/despesas', (req, res) => {execSQLQuery(sql, res)})

    //READ POR ID
    router.get('/receitasfixas/:id?', (req, res) => {
        let filter = ''
        if(req.params.id) filter = ' WHERE id_fixa = ' + req.params.id;
        execSQLQuery(sql + filter, res)})

    //DELETE POR ID
    router.delete('/receitasfixas/:id', (req, res) =>{
        execSQLQuery('DELETE FROM despesas where id_fixa = ' + parseInt(req.params.id), res)
    })

    //INSERIR 
    router.post('/receitasfixas', (req,res) =>{
        ...
        execSQLQuery(`INSERT INTO receitasfixas(id_categoria, id_cli_for, descricao, dataInicial, dataFinal , valor,tipoRepeticao) ` +
        ` values(${id_categoria},${id_cli_for},'${descricao}','${dataInicial}','${dataFinal}',${valor},'${tipoRepeticao}')`, res)
    })

    //ATUALIZAR
    router.patch('/receitasfixas/:id', (req,res) =>{
        ...
        const tipoRepeticao = req.body.tipoRepeticao.substring(0,2)
        execSQLQuery(`update receitasfixas set id_categoria = ${id_categoria}, id_cli_for = ${id_cli_for}, descricao = '${descricao}', dataInicial = '${dataInicial}', dataFinal = '${dataFinal}', `+
