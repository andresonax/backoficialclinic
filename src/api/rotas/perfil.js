...
module.exports = function(server) {
    ...
    //READ ALL
    var sql = 'SELECT * FROM perfis'
    router.get('/perfil', (req, res) => {execSQLQuery(sql, res)})

    //READ POR ID
    router.get('/perfis/:id?', (req, res) => {
        let filter = ''
        if(req.params.id) filter = ' WHERE codPerfil = ' + req.params.id;
